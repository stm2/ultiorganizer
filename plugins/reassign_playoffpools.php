...
//	die;
	...
	for($i=0;$i<(count($newmove1));$i++) {	
		$query=sprintf("UPDATE uo_moveteams SET topool='%s', torank='%s', scheduling_id='%s' 
			...
			mysql_adapt_real_escape_string($newmove1[$i]['topool']),
			mysql_adapt_real_escape_string($newmove1[$i]['torank']),
			mysql_adapt_real_escape_string($newmove1[$i]['scheduling_id']),
			mysql_adapt_real_escape_string($newmove1[$i]['frompool']),
			mysql_adapt_real_escape_string($newmove1[$i]['fromplacing'])	 );
			
//		echo $query."<br>";
		$result = mysql_adapt_query($query);
		if (!$result) { die('Invalid query: ' . mysql_adapt_error() . $query); }
				
		$query=sprintf("UPDATE uo_scheduling_name SET name='%s' 
						WHERE scheduling_id='%s'",
			mysql_adapt_real_escape_string($newmove1[$i]['pteamname']),
			mysql_adapt_real_escape_string($newmove1[$i]['scheduling_id']));			
//		echo $query."<br>";
		$result = mysql_adapt_query($query);
		if (!$result) { die('Invalid query: ' . mysql_adapt_error() . $query); }				
	...
	for($i=0;$i<(count($newmove2));$i++) {				
		$query=sprintf("UPDATE uo_moveteams SET topool='%s', torank='%s', scheduling_id='%s' 
						WHERE frompool='%s' AND fromplacing='%s'",
			mysql_adapt_real_escape_string($newmove2[$i]['topool']),
			mysql_adapt_real_escape_string($newmove2[$i]['torank']),
			mysql_adapt_real_escape_string($newmove2[$i]['scheduling_id']),
			mysql_adapt_real_escape_string($newmove2[$i]['frompool']),
			mysql_adapt_real_escape_string($newmove2[$i]['fromplacing'])	 );
		//		echo $query."<br>";
		$result = mysql_adapt_query($query);
		...
		$query=sprintf("UPDATE uo_scheduling_name SET name='%s' 
						WHERE scheduling_id='%s'",
			mysql_adapt_real_escape_string($newmove2[$i]['pteamname']),
			mysql_adapt_real_escape_string($newmove2[$i]['scheduling_id']));
//		echo $query."<br>";
		$result = mysql_adapt_query($query);
		if (!$result) { die('Invalid query: ' . mysql_adapt_error()); }
