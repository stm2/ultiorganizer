...
if (!empty($_POST['create'])) {
  ...
  while ($field = mysqli_fetch_assoc($fields)) {
    ...
    if (!empty($field['id']))
      $name = str($field['id']) . "_";
    $name .= intval($field['fieldname']);
    
    if (is_numeric($field['fieldname'])) {
      $name = "field" . $name;
    }
    $name = mysql_adapt_real_escape_string($name);
    ...
    $games = ReservationGamesByField($field['id'], $field['fieldname'], $season);
    while ($game = mysqli_fetch_assoc($games)) {
      $exist = DBQueryToValue(
        "SELECT COUNT(*) FROM uo_userproperties WHERE userid='$name' AND value='gameadmin:" . $game['game_id'] . "'");
      if ($user < 1) {
        DBQuery(
          "INSERT INTO uo_userproperties(userid, name, value) VALUES ('$name', 'userrole', 'gameadmin:" .
             $game['game_id'] . "')");
      }
    }
  }
