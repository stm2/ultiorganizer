...
class XMLHandler{
    ...
    if (is_array($attribs)) {
      ...
      while(list($key,$val) = each($attribs)) {
        if($val=="NULL"){
          ...
          $row[$key]=mysql_adapt_real_escape_string($val);
        }
      }
      switch($this->mode){
        case "new":
  ...
  function InsertToDatabase($name, $row){
    ...
    switch($name){
      ...
      case "DIVISION":
        ...
        $cond = "name='".mysql_adapt_real_escape_string($row['NAME'])."' AND season='".$this->eventId."'";
      ...
      case "TEAM":
        $cond = "name='".mysql_adapt_real_escape_string($row['NAME'])."' AND series='".$this->divisionId."'";
        ...
        if(!empty($email)){
          ...
          if(!$user){
			...
			$query = sprintf("INSERT INTO uo_userproperties (userid, name, value) VALUES ('%s', 'poolselector', 'currentseason')",
			  mysql_adapt_real_escape_string($email));
            DBQuery($query);
            $userid=mysql_adapt_real_escape_string($email);	  
          }else{
            $userid=$user['userid'];
          }
        }else{
      ...
      case "PLAYER":
        ...
        if(isset($row['FIRSTNAME'])){ 
          ...
          $cond = "firstname='".mysql_adapt_real_escape_string($row['FIRSTNAME'])."' AND lastname='".mysql_adapt_real_escape_string($row['LASTNAME'])."' AND team='".$this->teamId."'";
        }else{
          $row['LASTNAME']=stripslashes($row['LASTNAME']);
          $cond = "lastname='".mysql_adapt_real_escape_string($row['LASTNAME'])."' AND team='".$this->teamId."'";
        }
        ...
        if($profileId>0){
          ...
          $query = sprintf("SELECT pp.* FROM uo_player_profile pp
								...
								mysql_adapt_real_escape_string(strtoupper($row['LASTNAME'])));
  ...
  function SetRow($name, $row, $cond){
    ...
    $query = rtrim($query,', ');
    $query .= " WHERE ";
    $query .= mysql_adapt_real_escape_string($cond);
    return DBQueryInsert($query);
  }
}
