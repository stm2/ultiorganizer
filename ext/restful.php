...
if (isset($_SERVER['PATH_INFO'])) {
  ...
  $path = array_filter(explode("/", $_SERVER['ORIG_PATH_INFO']));
} else {
  $path = array();
}
...
if (count($path) == 0) {
  $objects = array("seasons", "series", "pools", "teams", "clubs", "countries", "players", "games", "results");
  ...
  if (isset($_SESSION) && isset($_SESSION['uid']) && $_SESSION['uid'] != "anonymous") {
    ...
    $output["user"]["link"] = $rf->getLink($_SESSION['uid'], $users);
  }
  foreach ($objects as $obj) {
    $output[$obj] = array("name" => _(ucwords($obj)), "link" => $rf->getLink(null, $obj));
  }
  echo "{\n \"objects\": " . encode_json($output) . "\n}";
} else {
  $object = $path[1];
  ...
  if (count($path) == 1) {
    ...
    echo encode_json($list);
  } elseif ($path[2] == "filters") {
    echo encode_json($restful->getFilters());
  } else {
    $item = $restful->getItem($path[2]);
    echo encode_json($item);
  }
}
