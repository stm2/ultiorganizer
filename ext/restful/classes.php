...
class Restful {
  ...
  function getLink($id = null, $class = null) {
    if ($class == null) {
      if (!isset($this->linkbase)) {
        $this->linkbase = (GetURLBase() . "/ext/restful.php/" . rawurlencode($this->getRestClassName()) . "/");
      }
      if (empty($id))
        return $this->linkbase;
      else
        return $this->linkbase . rawurlencode($id);
    } else {
      $objectName = ucwords($class);
      $object = new $objectName();
      return $object->getLink($id);
    }
  }
  ...
  function getItemSQL($id) {
    return sprintf($this->itemsql, mysql_adapt_real_escape_string($id));
  }
  ...
  function convertLinkFields(&$row, $short = false) {
    foreach ($this->linkfields as $field => $objectName) {
      if (!empty($row[$field])) {
        ...
        if ($short) {
          $nextLink = $object->getList(null, null, array(array('id' => $row[$field], 'name' => "Hodor")));
        } else {
...
$editable_seasons = array("join" => "or",
  "criteria" => array(
    array(
      "field" => array("constanttype" => "int",
        "value" => array("variable" => "userproperties", "key1" => "userrole", "key2" => "superadmin")),
      "operator" => ">", "value" => 0),
    array("field" => "season.season_id", "operator" => "in",
      "value" => array("variable" => "userproperties", "key1" => "userrole", "key2" => "seasonadmin",
        "implode-keys" => ","))));
$edit_seasons = array("field" => "season.season_id", "operator" => "in",
  "value" => array("variable" => "userproperties", "key1" => "editseason", "implode-keys" => ","));
$editing_seasons = array("join" => "and", "criteria" => array($editable_seasons, $edit_seasons));

$editable_series = array_copy($editable_seasons);
$editable_series["criteria"][] = array("field" => "series.series_id", "operator" => "in",
  "value" => array("variable" => "userproperties", "key1" => "userrole", "key2" => "seriesadmin", "implode-keys" => ","));
$editing_series = array("join" => "and", "criteria" => array($editable_series, $edit_seasons));

$editable_teams = array_copy($editable_seasons);
$editable_teams["criteria"][] = array("field" => "team.team_id", "operator" => "in",
  "value" => array("variable" => "userproperties", "key1" => "userrole", "key2" => "teamadmin", "implode-keys" => ","));
$editing_teams = array("join" => "and", "criteria" => array($editable_teams, $edit_seasons));
...
include_once $include_prefix . 'ext/restful/results.php';
include_once $include_prefix . 'ext/restful/clubs.php';
?>
