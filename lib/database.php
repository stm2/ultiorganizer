...
define('DB_VERSION', 87); //Database version matching to upgrade functions.
...
function CheckDB() {
  ...
  for ($i = $installedDb+1; $i <= DB_VERSION; $i++) {
    $upgradeFunc = 'upgrade'.$i;
    LogDbUpgrade($i);
    $upgradeFunc();
    $query = sprintf("insert into uo_database (version, updated) values (%d, now())", $i);
    runQuery($query);
    LogDbUpgrade($i, true);
  }
}
