...
function GameTeamResults($teamId, $poolId) {
  $query = sprintf("
		SELECT g.game_id, g.homescore, g.visitorscore, g.hasstarted, g.hometeam, g.visitorteam, COALESCE(pm.goals,0) AS scoresheet,
			...
			LEFT JOIN (SELECT COUNT(*) AS goals, game FROM uo_goal GROUP BY game) AS pm ON (g.game_id=pm.game)
			LEFT JOIN uo_scheduling_name sn ON(g.name=sn.scheduling_id)
			WHERE (g.hometeam=%d OR g.visitorteam=%d) AND g.pool=%d
			GROUP BY g.game_id",
...
function GameTeamScoreBoard($gameId, $teamId)
	{
	$query = sprintf("
...
function GameEvents($gameId){
	$query = sprintf("
		...
		FROM (SELECT time,ishome,'timeout' AS type FROM `uo_timeout` 
			WHERE game=%d UNION ALL SELECT time,ishome,type FROM uo_gameevent WHERE game=%d) AS tapahtuma 
		WHERE type!='media'
		ORDER BY time ",
		intval($gameId),
		intval($gameId));
...
function GameTurnovers($gameId)
	...
	$query = sprintf("
		SELECT time, ishome 
		FROM uo_gameevent 
		WHERE game=%d AND type='turnover' 
		ORDER BY time",
		intval($gameId));
...
function GameUpdateResult($gameId, $home, $away) {
  if (hasEditGameEventsRight($gameId)) {
    $query = sprintf(
        "UPDATE uo_game SET homescore=%d, visitorscore=%d, isongoing='1', hasstarted='1' WHERE game_id=%d", 
        intval($home), intval($away), intval($gameId));
    $result = mysql_adapt_query($query);
    if (!$result) {
...
function GameSetResult($gameId, $home, $away, $updatePools = true, $checkRights = true) {
  if (!$checkRights || hasEditGameEventsRight($gameId)) {
    ...
    $query = sprintf(
        "UPDATE uo_game SET homescore=%d, visitorscore=%d, isongoing='0', hasstarted='2' WHERE game_id=%d", 
        intval($home), intval($away), intval($gameId));
    $result = mysql_adapt_query($query);
    if (!$result) {
      die('Invalid query: ' . mysql_adapt_error());
...
function GameClearResult($gameId, $updatepools = true) {
  if (hasEditGameEventsRight($gameId)) {
    ...
    $query = sprintf(
        "UPDATE uo_game SET homescore=NULL, visitorscore=NULL, isongoing='0', hasstarted='0' WHERE game_id=%d", 
        intval($gameId));
    $result = mysql_adapt_query($query);
    if (!$result) {
      die('Invalid query: ' . mysql_adapt_error());
...
function GameSetDefenses($gameId, $home, $away) {
  if (hasEditGameEventsRight($gameId)) {
    $query = sprintf("UPDATE uo_game SET homedefenses=%d, visitordefenses=%d WHERE game_id=%d",
        intval($home),
        intval($away),
        intval($gameId));
    $result = mysql_adapt_query($query);
...
function GameAddNewPlayer($gameId, $firstname, $lastname, $accrid, $teamId, $number) {
	if (hasEditGamePlayersRight($gameId)) {
		...
		$playerId = mysql_adapt_insert_id();
...
function GameAddScore($gameId, $pass, $goal, $time, $number, $hscores, $ascores, $home, $iscallahan) {
	if (hasEditGameEventsRight($gameId)) {
		$query = sprintf("
			...
			mysql_adapt_real_escape_string($gameId),
			mysql_adapt_real_escape_string($number),
			mysql_adapt_real_escape_string($pass),
			mysql_adapt_real_escape_string($goal),
			mysql_adapt_real_escape_string($time),
			mysql_adapt_real_escape_string($hscores),
			mysql_adapt_real_escape_string($ascores),
			mysql_adapt_real_escape_string($home),
			mysql_adapt_real_escape_string($iscallahan),
			mysql_adapt_real_escape_string($pass),
			mysql_adapt_real_escape_string($goal),
			mysql_adapt_real_escape_string($time),
			mysql_adapt_real_escape_string($hscores),
...
function GameAddDefense($gameId, $player, $home, $caught, $time, $iscallahan, $number) {
	if (hasEditGameEventsRight($gameId)) {
		$query = sprintf("
			...
			mysql_adapt_real_escape_string($gameId),
			mysql_adapt_real_escape_string($number),
			mysql_adapt_real_escape_string($player),
			mysql_adapt_real_escape_string($time),
			mysql_adapt_real_escape_string($iscallahan),
			mysql_adapt_real_escape_string($caught),
			mysql_adapt_real_escape_string($home),
			mysql_adapt_real_escape_string($player),
			mysql_adapt_real_escape_string($time),
			mysql_adapt_real_escape_string($iscallahan),
...
function GameAddScoreEntry($uo_goal) {
	if (hasEditGameEventsRight($uo_goal['game'])) {
		$query = sprintf("
			...
			(game, num, assist, scorer, time, homescore, visitorscore, ishomegoal, iscallahan) 
			VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
			mysql_adapt_real_escape_string($uo_goal['game']),
			mysql_adapt_real_escape_string($uo_goal['num']),
			mysql_adapt_real_escape_string($uo_goal['assist']),
			mysql_adapt_real_escape_string($uo_goal['scorer']),
			mysql_adapt_real_escape_string($uo_goal['time']),
			mysql_adapt_real_escape_string($uo_goal['homescore']),
			mysql_adapt_real_escape_string($uo_goal['visitorscore']),
			mysql_adapt_real_escape_string($uo_goal['ishomegoal']),
			mysql_adapt_real_escape_string($uo_goal['iscallahan']));
...
function GameSetScoreSheetKeeper($gameId, $name) {
	if (hasEditGameEventsRight($gameId)) {
		...
		UPDATE uo_game 
		SET official='%s' 
		WHERE game_id=%d", mysql_adapt_real_escape_string($name), intval($gameId));
		}else {
			$query = sprintf("
		UPDATE uo_game
		SET official=NULL
		WHERE game_id=%d", intval($gameId));
		}		
	$result = mysql_adapt_query($query);
	if (!$result) { die('Invalid query: ' . mysql_adapt_error()); }
...
function GameSetHalftime($gameId, $time) {
	if (hasEditGameEventsRight($gameId)) {
		if (isset($time)) {
			...
			UPDATE uo_game 
			SET halftime='%s' 
			WHERE game_id=%d", mysql_adapt_real_escape_string($time), intval($gameId));
		}else {
			$query = sprintf("
			UPDATE uo_game 
			SET halftime=NULL 
...
function AddGame($params) {
	...
	if (hasEditGamesRight($poolinfo['series'])) {
		$query = sprintf("
			...
			VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s')",
			mysql_adapt_real_escape_string($params['hometeam']),
			mysql_adapt_real_escape_string($params['visitorteam']),
			mysql_adapt_real_escape_string($params['reservation']),
			mysql_adapt_real_escape_string($params['time']),
			mysql_adapt_real_escape_string($params['pool']),
			mysql_adapt_real_escape_string($params['valid']),
			mysql_adapt_real_escape_string($params['respteam']));
			
		$result = mysql_adapt_query($query);
		if (!$result) { die('Invalid query: ' . mysql_adapt_error()); }
	
		$id = mysql_adapt_insert_id();
		$query = sprintf("
