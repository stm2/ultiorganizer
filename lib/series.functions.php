...
function AddSeries($params) {
  if (hasEditSeasonSeriesRight($params['season'])) {
    ...
    mysql_adapt_real_escape_string($params['season']),
    (int)$params['valid'],
...
function AddSeriesEnrolledTeam($seriesId, $userid, $name, $club, $country) {
  ...
  if ($userid == $_SESSION['uid'] || hasEditTeamsRight($seriesId)) {
    ...
    mysql_adapt_real_escape_string($userid),
    mysql_adapt_real_escape_string($name),
    mysql_adapt_real_escape_string($club),
...
function ConfirmEnrolledTeam($seriesId, $id) {
  if (hasEditTeamsRight($seriesId)) {
    ...
    mysql_adapt_real_escape_string($teaminfo['name']),
    (int)$seriesId);
    ...
    if($countryId && !$clubId){
      ...
      DBQuery("UPDATE uo_team SET abbreviation=UPPER('".mysql_adapt_real_escape_string($abb)."') WHERE team_id=$teamId");
    }
  ...
  if (isSeasonAdmin(SeriesSeasonId($to))) {
    ...
    foreach($teams as $team){
      $query = sprintf("INSERT INTO uo_team(name, club, country, rank, abbreviation, valid, series )
          ...
          (int) $team['country'],
          (int) $team['rank'],
          mysql_adapt_real_escape_string($team['abbreviation']),
          (int) $to);
       DBQuery($query);
    }
  } else { die('Insufficient rights'); }
