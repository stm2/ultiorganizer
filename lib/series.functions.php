 ...
 *          set TRUE if only visible pools are returned.
 * @param boolean $nocontinuingpools
 *          set TRUE if continuation pools are exluded.
 * @param boolean $noplacementpools
 *          set TRUE if placement pools are exluded.
 ...
 * @param int $seriesId
 *          uo_series.series_id
 * @param boolean $orderbyseeding
 *          TRUE if order by seeding otherwise order by name.
...
function SeriesTeamsWithoutPool($seriesId) {
  $query = sprintf(
    "SELECT pj.team_id, pj.name, pj.club, club.name as clubname, pj.`rank` 
		...
		ORDER BY pj.`rank` ASC", (int) $seriesId);
  return DBQueryToArray($query);
}
...
function Series($filter = null, $ordering = null) {
  ...
  $query = sprintf(
    "SELECT series_id, series.name as name, season.name as seasonname, series.season
 ...
 *          one of: "total", "goal", "pass", "games", "team", "name", "callahan"
 * @param int $limit
 *          Numbers of rows returned, 0 if unlimited
...
function SeriesScoreBoard($seriesId, $sorting, $limit) {
  $query = sprintf(
    "
		...
		WHERE pel.games > 0 AND j.series=%d", (int) $seriesId, (int) $seriesId, (int) $seriesId, (int) $seriesId,
    (int) $seriesId);
 ...
 *          uo_series.series_id
 * @param string $sorting
 *          one of: "total", "games", "team", "name", "callahan"
 * @param int $limit
 *          Numbers of rows returned, 0 if unlimited
 * @return array mysql array of players.
 */
  ...
  $query = sprintf(
    "
		...
		LEFT JOIN (SELECT up.player, COUNT(*) AS games 
			...
			WHERE pool.series=%d AND g4.isongoing=0 
			GROUP BY player) AS pel ON (p.player_id=pel.player) 
		WHERE pel.games > 0 AND j.series=%d", (int) $seriesId, (int) $seriesId, (int) $seriesId);
...
function SeriesInfo($seriesId) {
  $query = sprintf("SELECT * FROM uo_series WHERE series_id=%d", (int) $seriesId);
  ...
  if (hasEditTeamsRight($seriesId)) {
    $query = sprintf(
      "SELECT team.*, user.name AS username FROM uo_enrolledteam team
		...
		WHERE team.series=%d ORDER BY team.enroll_time ASC", (int) $seriesId);
    return DBQuery($query);
  } else
  ...
  if (hasEditSeasonSeriesRight($params['season'])) {
    $query = sprintf(
      "INSERT INTO uo_series
				...
				VALUES ('%s','%s','%s','%s',%d,%d)", mysql_adapt_real_escape_string($params['name']),
      mysql_adapt_real_escape_string($params['type']), mysql_adapt_real_escape_string($params['ordering']),
      mysql_adapt_real_escape_string($params['season']), (int) $params['valid'], (int) $params['pool_template']);
  ...
  if (hasEditSeasonSeriesRight($seriesInfo['season'])) {
    $query = sprintf(
      "
			...
			pool_template=%d
			WHERE series_id=%d", mysql_adapt_real_escape_string($params['name']), mysql_adapt_real_escape_string($params['type']),
      mysql_adapt_real_escape_string($params['ordering']), (int) $params['valid'], (int) $params['pool_template'],
...
function SetSeriesName($seriesId, $name) {
  ...
  if (hasEditSeasonSeriesRight($seriesInfo['season'])) {
    $query = sprintf("
			UPDATE uo_series SET name='%s' WHERE series_id='%s'", mysql_adapt_real_escape_string($name),
...
function SeriesEnrolledTeamById($seriesId, $id) {
  if (hasEditTeamsRight($seriesId)) {
    $query = sprintf("SELECT * FROM uo_enrolledteam WHERE series=%d and id=%d", (int) $seriesId, (int) $id);
...
function SeriesEnrolledTeamsByUser($seriesId, $userid) {
  ...
  if ($userid == $_SESSION['uid'] || hasEditTeamsRight($seriesId)) {
    $query = sprintf(
      "SELECT team.*, user.name AS username FROM uo_enrolledteam team
				LEFT JOIN uo_users user ON(team.userid=user.userid)
				WHERE team.series=%d and team.userid='%s' ORDER BY team.enroll_time ASC", (int) $seriesId,
 ...
 *          uo_series.series_id
 * @param string $userid
 *          uo_user.userid will be granted as team admin.
 * @param string $name
 *          name of team
 * @param int $club
 *          uo_club.club_id
 * @param int $country
 *          uo_country.country_id
  ...
  if ($userid == $_SESSION['uid'] || hasEditTeamsRight($seriesId)) {
    $query = sprintf(
      "INSERT INTO uo_enrolledteam (series, userid, name, clubname, countryname, enroll_time)
				VALUES (%d, '%s', '%s', '%s', '%s', now())", (int) $seriesId, mysql_adapt_real_escape_string($userid),
      mysql_adapt_real_escape_string($name), mysql_adapt_real_escape_string($club),
      mysql_adapt_real_escape_string($country));
    $id = DBQueryInsert($query);
...
function RemoveSeriesEnrolledTeam($seriesId, $userid, $id) {
  ...
  if ($userid == $_SESSION['uid'] || hasEditTeamsRight($seriesId)) {
    if (!hasEditTeamsRight($seriesId)) {
      ...
      $query = sprintf($query, (int) $seriesId, mysql_adapt_real_escape_string($userid), (int) $id);
    } else {
      $query = "DELETE FROM uo_enrolledteam WHERE series=%d and id=%d";
      $query = sprintf($query, (int) $seriesId, (int) $id);
    }
...
function ConfirmEnrolledTeam($seriesId, $id) {
  if (hasEditTeamsRight($seriesId)) {
    ...
    $query = sprintf("INSERT INTO uo_team (name, series, valid) VALUES ('%s', %d, 1)",
      mysql_adapt_real_escape_string($teaminfo['name']), (int) $seriesId);
    ...
    if ($countryId && !$clubId) {
      ...
      DBQuery(
        sprintf("UPDATE uo_team SET abbreviation=UPPER('%s') WHERE team_id=%d",
          mysql_adapt_real_escape_string($countryinfo['abbreviation']), (int) $teamId));
    } else {
    ...
    die("Insufficient rights to confirm enrolled teams");
}
...
function CanDeleteSeries($seriesId) {
  $query = sprintf("SELECT count(*) FROM uo_pool WHERE series='%s'", mysql_adapt_real_escape_string($seriesId));
  ...
  if (!$row = mysqli_fetch_row($result))
    return false;
  if ($row[0] == 0) {
    $query = sprintf("SELECT count(*) FROM uo_team WHERE series='%s'", mysql_adapt_real_escape_string($seriesId));
    $result = mysql_adapt_query($query);
    if (!$result) {
      die('Invalid query: ' . mysql_adapt_error());
    }
    ...
    foreach ($teams as $team) {
      $query = sprintf(
        "INSERT INTO uo_team(name, club, country, `rank`, abbreviation, valid, series )
      			VALUES ('%s',%d,%d,%d,'%s',1,%d)", mysql_adapt_real_escape_string($team['name']), (int) $team['club'],
        (int) $team['country'], (int) $team['rank'], mysql_adapt_real_escape_string($team['abbreviation']), (int) $to);
      DBQuery($query);
    }
  } else {
    die('Insufficient rights');
