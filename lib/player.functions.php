...
function SetPlayer($playerId, $number, $fname, $lname, $accrId, $profileId) {
  ...
  if (hasEditPlayersRight($playerInfo['team'])) {
    ...
    $query = sprintf("UPDATE uo_player SET num=%s, firstname='%s', lastname='%s', accreditation_id='%s',
			...
			WHERE player_id=%d",
    $number,
    mysql_adapt_real_escape_string($fname),
    mysql_adapt_real_escape_string($lname),
    mysql_adapt_real_escape_string($accrId),
    mysql_adapt_real_escape_string($profileId),
    (int)$playerId);
...
function CreatePlayerProfile($playerId) {
  ...
  if (hasEditPlayersRight($playerInfo['team']) && !empty($playerInfo)) {
    ...
    $query = sprintf("INSERT INTO uo_player_profile (firstname,lastname,accreditation_id,num) VALUES
				('%s','%s','%s','%s')",
    mysql_adapt_real_escape_string($playerInfo['firstname']),
    mysql_adapt_real_escape_string($playerInfo['lastname']),
    mysql_adapt_real_escape_string($playerInfo['accreditation_id']),
    mysql_adapt_real_escape_string($playerInfo['num']));
...
function SetPlayerProfile($teamId, $playerId, $profile) {
  ...
  if (hasEditPlayerProfileRight($playerId) && $playerInfo['team']==$teamId) {
    ...
    $query = sprintf("UPDATE uo_player SET num=%s, firstname='%s', lastname='%s', accreditation_id='%s'
			WHERE player_id=%d",
        $number,
        mysql_adapt_real_escape_string($profile['firstname']),
        mysql_adapt_real_escape_string($profile['lastname']),
        mysql_adapt_real_escape_string($profile['accreditation_id']),
        (int)$playerId);
    ...
    if(!$exist){
      ...
      mysql_adapt_real_escape_string($profile['firstname']),
      mysql_adapt_real_escape_string($profile['lastname']),
      mysql_adapt_real_escape_string($profile['num']),
      mysql_adapt_real_escape_string($profile['email']),
      mysql_adapt_real_escape_string($profile['nickname']),
      mysql_adapt_real_escape_string($profile['gender']),
      mysql_adapt_real_escape_string($profile['info']),
      mysql_adapt_real_escape_string($profile['national_id']),
      mysql_adapt_real_escape_string($profile['birthdate']),
      mysql_adapt_real_escape_string($profile['birthplace']),
      mysql_adapt_real_escape_string($profile['nationality']),
      mysql_adapt_real_escape_string($profile['throwing_hand']),
      mysql_adapt_real_escape_string($profile['height']),
      mysql_adapt_real_escape_string($profile['weight']),
      mysql_adapt_real_escape_string($profile['position']),
      mysql_adapt_real_escape_string($profile['story']),
      mysql_adapt_real_escape_string($profile['achievements']),
      mysql_adapt_real_escape_string($profile['public']));
    ...
    DBQuery($query);
      ...
      mysql_adapt_real_escape_string($profile['achievements']),
      mysql_adapt_real_escape_string($profile['public']),
      mysql_adapt_real_escape_string($profile['profile_id']));
