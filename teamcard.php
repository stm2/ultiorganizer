...
if (intval($teaminfo['country']) > 0) {
  ...
  $html .= "<a class='headerlink' href='?view=countrycard&amp;country=" . $teaminfo['country'] . "'>" .
    utf8entities($teaminfo['countryname']) . "</a></p>";
}
if (intval($teaminfo['club']) > 0) {
  $html .= "<p>" . _("Club") . ": <a class='headerlink' href='?view=clubcard&amp;club=" . $teaminfo['club'] . "'>" .
    utf8entities($teaminfo['clubname']) . "</a></p>";
}
if ($profile) {
  ...
  if (!empty($profile['profile_image'])) {
    ...
    $html .= "<img src='" . UPLOAD_DIR . "teams/$teamId/thumbs/" . $profile['profile_image'] . "' alt='" .
      _("Profile image") . "'/></a></td></tr>\n";
  } else {
...
if (ShowDefenseStats()) {
  ...
  if (mysqli_num_rows($playerswihtdef)) {
    $html .= "<p><span class='profileheader'>" . utf8entities(U_(SeasonName($teaminfo['season']))) . " " . _("roster") .
      ":</span></p>\n";
    ...
    while ($player = mysqli_fetch_assoc($playerswihtdef)) {
      ...
      if (!empty($playerinfo['profile_id'])) {
        ...
        if ($playerinfo['num'] > -1) {
          $html .= "#" . $playerinfo['num'] . " " .
            utf8entities($playerinfo['firstname'] . " " . $playerinfo['lastname']);
        } else {
...
if (mysqli_num_rows($allgames)) {
  $gameshtml .= "<h2>" . U_(SeasonName($teaminfo['season'])) . ":</h2>\n";
  $gameshtml .= "<p>" . _("Division") . ": <a href='?view=poolstatus&amp;series=" . $teaminfo['series'] . "'>" .
    utf8entities(U_($teaminfo['seriesname'])) . "</a></p>";
  $gameshtml .= "<table style='width:80%'>\n";
...
if (ShowDefenseStats()) {
  if (count($seasons)) {
    ...
    foreach ($seasons as $season) {
      ...
      // played games
      $pg = array("season_type" => "", "games" => 0, "wins" => 0, "losses" => 0, "goals_made" => 0, "goals_against" => 0,
        "defenses" => 0);
  ...
  while ($row = mysqli_fetch_assoc($played)) {
    ...
    if (GameHasStarted($row)) {
      ...
      if (false) {
        ...
        $html .= "<td><a href=\"?view=gameplay&amp;game=" . $row['game_id'] . "\">" . $row['homescore'] . " - " .
          $row['visitorscore'] . "</a></td>";

        $html .= "<td>" . utf8entities(U_($seasonName)) . ": <a href=\"?view=poolstatus&amp;pool=" . $row['pool_id'] .
          "\">" . utf8entities(U_($row['name'])) . "</a></td></tr>";
      }
    }
  }
...
if ($_SESSION['uid'] != 'anonymous') {
  $html .= "<div style='float:left;'><hr/><a href='?view=user/addmedialink&amp;team=$teamId'>" . _("Add media") .
    "</a></div>";
}
showPage($title, $html);
