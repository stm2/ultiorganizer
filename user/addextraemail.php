...
if (!empty($_POST['add'])) {
  ...
  if ($error == 0) {
    if (AddExtraEmailRequest($userid, $newEmail)) {
      ...
      $message .= "<p>" . _("Confirmation email could not be sent.") . "</p>\n";
    }
  } else {
...
if (!empty($_GET['token'])) {
  if (ConfirmEmail($_GET['token'])) {
    $message = "<p>" . _("Extra email address was confirmed successfully.") . "</p>\n";
    $confirmed = true;
  } else {
    $message = "<p>" . _("Confirming extra email address failed.") . "</p>\n";
  }
}
