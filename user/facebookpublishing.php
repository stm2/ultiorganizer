...
$title = _("Manage player Facebook feeds for") . " " .
   utf8entities($playerInfo['firstname'] . " " . $playerInfo['lastname']);

if (!IsFacebookEnabled()) {
  $html = "<p>" . _("Facebook is not enabled on this site!");
  showPage($title, $html);
} else {
  ...
  if (isset($_POST['publish'])) {
    ...
    $params = array("link" => GetURLBase() . "?view=playercard&Player=" . $latestId,
      "message" => $_POST['publishmessage'], "name" => $linktitle
    );
  ...
  $help = "<p>" . _("Facebook feed messages") . ":</p>
	<ol>
		<li> " .
     _("Messages will be published on your feed when the event happens") . " </li>
		<li> " .
     _("Placeholders will be replaced from with information from the events") . " </li>
		<li> " . _(
      "The available placeholders are the following:") . " 
		<ul>
			...
			<li><b>\$passername</b> " .
     _("Name of the player passing the goal (goal events only)") . "</li>
  ...
  foreach ($events as $event => $message) {
    ...
    echo "/> " . "</td></tr>\n";
    echo "<tr><th>" . _("Message") . "</th><td><textarea name='message$event' rows='3' cols='50'>" .
       $fb_props['facebookplayer'][$playerid][$event . "message"] . "</textarea></td></tr>\n";
  }
  echo "</table><input type='submit' name='save' value='" . _("Save") . "'/></form>\n";
  echo "<h2>" . _("Publish player card") . "</h2>\n";
  echo "<form method='post' action='?view=user/facebookpublishing&amp;player=$playerid'>\n";
  echo "<table>\n";
  echo "<tr><th>" . _("Message") . "</th><td><textarea name='publishmessage' rows='3' cols='50'>" .
     _("Check out my player profile in ultiorganizer") . "</textarea></td></tr>\n";
