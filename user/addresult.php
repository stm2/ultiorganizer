...
if (!empty($_POST['save'])) {
  ...
  if ($ok) {
    ...
    if ($home > $away) {
      $html2 .= sprintf(_("Winner is <span style='font-weight:bold'>%s</span>."),
        utf8entities($game_result['hometeamname']));
    } elseif ($away > $home) {
      $html2 .= sprintf(_("Winner is <span style='font-weight:bold'>%s</span>."),
        utf8entities($game_result['visitorteamname']));
    }
    $html2 .= "</p>";
  }
  $game_result = GameInfo($gameId);
...
$html .= "<form  method='post' action='?view=user/addresult&amp;game=" . $gameId .
  "'>
<table cellpadding='2'>
<tr><td><b>" . utf8entities($game_result['hometeamname']) . "</b></td><td><b> - </b></td><td><b>" .
  utf8entities($game_result['visitorteamname']) . "</b></td></tr>";
...
$html .= "<tr>
<td><input class='input' name='home' value='" . utf8entities($game_result['homescore']) .
  "' maxlength='4' size='5'/></td>
<td> - </td>
<td><input class='input' name='away' value='" . utf8entities($game_result['visitorscore']) .
  "' maxlength='4' size='5'/></td></tr>
</table>";

if ($game_result['homevalid'] == 2) {
  $poolInfo = PoolInfo($game_result['pool']);
  $html .= "<p>" . "The home team is the BYE team. You should use the suggested result: " . $poolInfo['forfeitagainst'] .
    " - " . $poolInfo['forfeitscore'] . "</p>";
} elseif ($game_result['visitorvalid'] == 2) {
  $poolInfo = PoolInfo($game_result['pool']);
  $html .= "<p>" . "The visitor team is the BYE team. You should use the suggested result: " . $poolInfo['forfeitscore'] .
    " - " . $poolInfo['forfeitagainst'] . "</p>";
}
...
if (hasEditGameEventsRight($gameId)) {
  ...
  $html .= "<p><input class='button' type='submit' name='save' value='" . _("Save as final result") . "'/></p>";
} else {
